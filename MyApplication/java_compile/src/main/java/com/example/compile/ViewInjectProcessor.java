...
public class ViewInjectProcessor extends AbstractProcessor {
    ...
    void writeToFile() {
            ...
            for (String classFullName : classMap.keySet()) {
                ...
                for (VariableInfo variableInfo : variableList) {
                    ...
                    constructor.addStatement("activity.$L=($L)activity.findViewById($L)",
                            variableName, variableFullName, variableInfo.viewId);
                }
        ...
        try {
            TypeSpec typeSpec = TypeSpec.classBuilder("Hello$$ViewInjector")
                .addModifiers(Modifier.PUBLIC)

                .build();

            // 与目标Class放在同一个包下，解决Class属性的可访问性
              JavaFile javaFile = JavaFile.builder("com.my.hello", typeSpec)
                    .build();
            // 生成class文件
            javaFile.writeTo(filer);
