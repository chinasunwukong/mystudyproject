...
public class BitmapBlurUtil {
    ...
    public void setBlurBackground(final Context context, final View view, final Bitmap bitmap) {
        if (bitmap == null || context == null) return;
        final WeakReference<Context> weakReference = new WeakReference<Context>(context);
        executorService.execute(new Runnable() {
            @Override
            public void run() {
                if (weakReference.get() != null){
                    final Bitmap blurBitmap = blur(weakReference.get(), bitmap);
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            if (view != null && weakReference.get() != null){
                                BitmapDrawable bd= new BitmapDrawable(context.getResources(), blurBitmap);
